...
export default function TenantsClient() {
  ...
  const [residentStatusFilter, setResidentStatusFilter] = useState<
    "All" | "Active" | "Inactive" | "Invite Pending"
  >("All");
  const [chefStatusFilter, setChefStatusFilter] = useState<
  ...
  const [chefShift, setChefShift] = useState<"Morning" | "Evening" | "Split">(
    "Morning",
  );
  ...
  const filteredResidents = useMemo(() => {
    return currentDormTenants.filter((tenant) => {
      ...
      const matchesStatus =
        ...
        (residentStatusFilter === "Invite Pending" &&
  ...
  const filteredChefs = useMemo(() => {
    return currentDormChefs.filter((chef) => {
      ...
      const matchesStatus =
        chefStatusFilter === "All" || chef.status === chefStatusFilter;
      return matchesSearch && matchesStatus;
    });
  ...
  const residentCounts = useMemo(
    () => ({
      ...
      active: currentDormTenants.filter((tenant) => tenant.status === "Active")
        .length,
      awaitingRoom: currentDormTenants.filter(
        (tenant) =>
          tenant.status === "Active" && tenant.roomId === "unassigned",
      ).length,
      inactive: currentDormTenants.filter(
        (tenant) =>
          tenant.status === "Inactive" &&
          tenant.invitationLifecycleState !== "pending",
      ).length,
      pending: currentDormTenants.filter(
  ...
  const chefCounts = useMemo(
    () => ({
      ...
      active: currentDormChefs.filter((chef) => chef.status === "Active")
        .length,
      invited: currentDormChefs.filter((chef) => chef.status === "Invited")
        .length,
    }),
    [currentDormChefs],
  );
  ...
  const residentRoomOptions = useMemo(
    () => [
      { value: "unassigned", label: "No room assigned" },
      ...currentDormRooms.map((room) => ({
        value: room.id,
        label: `Room ${room.roomNumber} · ${room.occupants}/${room.capacity} occupied`,
      })),
    ],
    [currentDormRooms],
  );
  ...
  useEffect(() => {
    ...
    const residentStillVisible = currentDormTenants.some(
      (tenant) => tenant.id === movingResidentId,
    );
  ...
  function handleAddResident(event: React.FormEvent) {
    ...
    try {
      ...
      toast.success(`${result.resident.name} added.`);
      toast.info(`Invitation code: ${result.invitationCode}`);
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Unable to add the resident.";
      toast.error(message);
    }
  }
  ...
  function handleAddChef(event: React.FormEvent) {
    ...
    try {
      ...
      toast.success(`${result.chef.name} invited.`);
      toast.info(`Invitation code: ${result.invitationCode}`);
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Unable to invite the chef.";
      toast.error(message);
    }
  }

  function handleResidentStatusToggle(
    tenantId: string,
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to update the resident status.";
      toast.error(message);
    }
  }

  function openResidentReassignment(
    tenant: (typeof currentDormTenants)[number],
  ) {
  ...
  function handleResidentReassignment(
    tenant: (typeof currentDormTenants)[number],
  ) {
    if (!nextResidentRoomId) {
    ...
    try {
      const targetRoom = currentDormRooms.find(
        (room) => room.id === nextResidentRoomId,
      );
  ...
  function handleChefStatusChange(
    chefId: string,
    ...
    try {
      ...
      toast.success(
        `Kitchen staff status changed to ${nextStatus.toLowerCase()}`,
      );
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Unable to update the kitchen staff status.";
      toast.error(message);
    }
  }
  ...
  function handleChefReInvite(chefId: string) {
    try {
      ...
      toast.success(`${result.chef.name} invited again.`);
      toast.info(`Invitation code: ${result.invitationCode}`);
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Unable to re-invite the chef.";
      toast.error(message);
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-[14px] text-[hsl(var(--muted-foreground))]">
            {currentDorm?.name ?? "Dorm"} · manage residents, room assignments,
            and kitchen staff.
          </p>
        </div>
        <button
          ...
          {activeView === "residents"
            ...
            : mealServiceEnabled
              ...
              : chefManagementAccess?.reason === "plan"
                ? "Upgrade for kitchen staff"
                : "Turn on meal service"}
        </button>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl bg-[hsl(var(--primary))] p-5 text-white">
          <p className="text-[12px] font-medium uppercase tracking-[0.18em] text-white/75">
            Residents
          </p>
          <p className="mt-3 text-3xl font-semibold">{residentCounts.total}</p>
          <p className="mt-1 text-[13px] text-white/75">
            {residentCounts.active} active residents
            {residentCounts.awaitingRoom > 0
              ? ` · ${residentCounts.awaitingRoom} awaiting rooms`
              : ""}
          </p>
        </div>
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-5">
          <p className="text-[12px] font-medium uppercase tracking-[0.18em] text-amber-700">
            Pending Resident Actions
          </p>
          <p className="mt-3 text-3xl font-semibold text-amber-900">
            {residentCounts.pending +
              residentCounts.inactive +
              residentCounts.awaitingRoom}
          </p>
          <p className="mt-1 text-[13px] text-amber-700">
            Pending invites, inactive records, or missing room assignments
          </p>
        </div>
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
          <p className="text-[12px] font-medium uppercase tracking-[0.18em] text-blue-700">
            Kitchen Staff
          </p>
          ...
          <p className="mt-1 text-[13px] text-blue-700">
            {mealServiceEnabled
              ...
              : chefManagementAccess?.reason === "plan"
                ? "Premium is required for kitchen staff tools"
                : "Meal service is turned off"}
          </p>
        </div>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
          <p className="text-[12px] font-medium uppercase tracking-[0.18em] text-emerald-700">
            Current Setup
          </p>
          ...
          <p className="mt-1 text-[13px] text-emerald-700">
            Resident tools stay available. Kitchen staff tools follow meal
            service.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        {activeView === "residents" ? (
          <div className="flex gap-2">
            {(["All", "Active", "Inactive", "Invite Pending"] as const).map(
              (status) => (
                <button
                  key={status}
                  type="button"
          ...
          <div className="flex gap-2">
            {(["All", "Active", "Invited", "Inactive"] as const).map(
              (status) => (
                <button
                  key={status}
                  type="button"
      ...
      {activeView === "kitchen" && !mealServiceEnabled && (
        <FeatureGateNotice
          ...
          title={
            chefManagementAccess?.reason === "plan"
              ? "Kitchen staff tools require Premium"
              : "Kitchen staff tools are turned off for this dorm"
          }
          description={
            chefManagementAccess?.reason === "plan"
              ? "Upgrade this dorm to invite kitchen staff and use the kitchen dashboard."
              : "Turn meal service back on in settings to invite kitchen staff and manage meals."
          }
          ctaLabel={
            chefManagementAccess?.reason === "plan"
              ? "Upgrade to Premium"
      ...
      {showResidentForm && activeView === "residents" && (
        <form
          ...
          className="space-y-4 rounded-xl border border-[hsl(var(--border))] bg-white p-6"
        >
          <div>
            ...
            <p className="mt-1 text-[13px] text-[hsl(var(--muted-foreground))]">
              Create a resident record and optionally reserve a room now.
            </p>
          </div>
      ...
      {showChefForm && activeView === "kitchen" && mealServiceEnabled && (
        ...
        >
          <div>
            ...
            <p className="mt-1 text-[13px] text-[hsl(var(--muted-foreground))]">
              Add a kitchen staff account for this dorm.
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <AppSelect
                ...
                onChange={(value) =>
                  setChefShift(value as "Morning" | "Evening" | "Split")
                }
              />
            </div>
            <div className="space-y-1.5">
      ...
      {activeView === "residents" ? (
        <div className="overflow-hidden rounded-xl border border-[hsl(var(--border))] bg-white">
          <div className="border-b border-[hsl(var(--border))] px-5 py-4">
            ...
            <p className="mt-0.5 text-[13px] text-[hsl(var(--muted-foreground))]">
              {activeViewCount} resident{activeViewCount === 1 ? "" : "s"} match
              the current filter.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[hsl(var(--muted)/0.45)]">
                <tr>
                  <th className="px-5 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">
                    Resident
                  </th>
                  <th className="px-5 py-3 text-left text-[12px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">
                    Contact
                  ...
                  <th className="px-5 py-3 text-right text-[12px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[hsl(var(--border))]">
                {filteredResidents.map((tenant) => {
                  const moveOptions =
                    residentReassignmentOptions.get(tenant.id) ?? [];
                  ...
                  return (
                    <React.Fragment key={tenant.id}>
                      <tr className="hover:bg-[hsl(var(--muted)/0.3)] transition-colors">
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12px] font-medium ${statusMeta.badge}`}
                          >
                            <StatusIcon size={11} />
                            {statusMeta.label}
                          </span>
                            ...
                            <div className="rounded-xl border border-[hsl(var(--border))] bg-white p-4">
                              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                                <div>
                                  ...
                                  <p className="mt-1 text-[13px] text-[hsl(var(--muted-foreground))]">
                                    Current room: {getRoomLabel(tenant.roomId)}.
                                    Choose a room with available space.
                                  </p>
                                </div>
                                <div className="flex flex-col gap-3 md:flex-row md:items-center">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleResidentReassignment(tenant)
                                      }
        ...
        <div className="overflow-hidden rounded-xl border border-[hsl(var(--border))] bg-white">
          <div className="border-b border-[hsl(var(--border))] px-5 py-4">
            ...
            <p className="mt-0.5 text-[13px] text-[hsl(var(--muted-foreground))]">
              {activeViewCount} kitchen staff record
              {activeViewCount === 1 ? "" : "s"} match the current filter.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
                        ...
                        {mealServiceEnabled &&
                          ...
                          chef.status !== "Active" && (
                            <button
                              type="button"
                              onClick={() =>
                                handleChefStatusChange(chef.id, "Active")
                              }
                              className="text-[12px] font-medium text-[hsl(var(--primary))] hover:underline"
                            >
                              Activate
                        ...
                        {mealServiceEnabled &&
                          ...
                          chef.status !== "Inactive" && (
                            <button
                              type="button"
                              onClick={() =>
                                handleChefStatusChange(chef.id, "Inactive")
                              }
          ...
          {filteredChefs.length === 0 && (
            <div className="px-6 py-12 text-center">
              <p className="text-[14px] text-[hsl(var(--muted-foreground))]">
                No kitchen staff match this filter
              </p>
            </div>
          )}
        </div>
