...
export default function MultiDormPage() {
  ...
  const portfolio = useMemo(
    () =>
      activeDorms.map((dorm) => {
        ...
        const invoices = workspace.invoices.filter(
          (invoice) => invoice.dormId === dorm.id,
        );
        const maintenance = workspace.maintenanceTickets.filter(
          (ticket) => ticket.dormId === dorm.id && ticket.status !== "Resolved",
        );
    ...
    [
      activeDorms,
      workspace.invoices,
      workspace.maintenanceTickets,
      workspace.rooms,
    ],
  );

  const totalCapacity = portfolio.reduce((sum, item) => sum + item.capacity, 0);
  const totalOccupiedBeds = portfolio.reduce(
    (sum, item) => sum + item.occupiedBeds,
    0,
  );
  const totalRevenue = portfolio.reduce(
    (sum, item) => sum + item.monthlyRevenue,
    0,
  );
  const totalMaintenance = portfolio.reduce(
    (sum, item) => sum + item.openMaintenance,
    0,
  );
  const totalWaitlist = activeDorms.reduce(
    (sum, dorm) => sum + dorm.waitlist,
    0,
  );
  const occupancyRate =
    totalCapacity > 0
      ? Math.round((totalOccupiedBeds / totalCapacity) * 100)
      : 0;
  const averageCollectionRate =
    portfolio.length > 0
      ? Math.round(
          portfolio.reduce((sum, item) => sum + item.collectionRate, 0) /
            portfolio.length,
        )
      : 0;
  ...
  function handleAddDorm(event: React.FormEvent) {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to add the dorm.";
      toast.error(message);
    }
  }
  ...
  function handleArchiveDorm(dormId: string, dormName: string) {
    ...
    if (!didArchive) {
      toast.info("At least one active dorm must remain.");
      return;
    }

    toast.success(`${dormName} archived.`);
  }
  ...
  function activateDormWorkspace(dormId: string, successMessage?: string) {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to switch dorm.";
      toast.error(message);
      return false;
    }
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-0.5 text-[14px] text-[hsl(var(--muted-foreground))]">
            Manage more than one dorm when this dorm has Premium and Multiple
            Dorms turned on.
          </p>
        </div>
        <FeatureGateNotice
          eyebrow={
            multiDormAccess.reason === "plan" ? "Premium feature" : "Module off"
          }
          title={
            multiDormAccess.reason === "plan"
              ? "Multiple dorms require Premium"
              : "Multiple dorms are turned off for this dorm"
          }
          description={
            multiDormAccess.reason === "plan"
              ? "Upgrade this dorm to add more dorms and switch between them."
              : "Turn Multiple Dorms back on in settings to add dorms and switch between them."
          }
          ctaLabel={
            multiDormAccess.reason === "plan"
              ? "Upgrade to Premium"
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[14px] text-[hsl(var(--muted-foreground))]">
            Add dorms, switch the active dorm, and review each dorm from one
            owner account.
          </p>
        </div>
      ...
      {showForm && (
        <form
          onSubmit={handleAddDorm}
          className="grid gap-4 rounded-xl border border-[hsl(var(--border))] bg-white p-6 md:grid-cols-2 xl:grid-cols-5"
        >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <p className="mt-1 text-[13px] text-emerald-700">
            {activeDorms.length} active dorms tracked
          </p>
        </div>
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
          ...
          <p className="mt-1 text-[13px] text-blue-700">Across active dorms</p>
        </div>
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mt-1 text-[13px] text-amber-700">
            {totalWaitlist} residents on waitlists
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.65fr_1fr]">
        <div className="rounded-xl border border-[hsl(var(--border))] bg-white p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-[hsl(var(--muted-foreground))]">
                Make a dorm active before managing its rooms, residents,
                invoices, maintenance, or settings.
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-4">
            {portfolio.map((item) => {
              const dormOccupancy =
                item.capacity > 0
                  ? Math.round((item.occupiedBeds / item.capacity) * 100)
                  : 0;
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {!isActiveDorm && (
                      <button
                        type="button"
                        onClick={() => {
                          activateDormWorkspace(
                            item.dorm.id,
                            `${item.dorm.name} is now the active workspace`,
                          );
                        }}
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        handleArchiveDorm(item.dorm.id, item.dorm.name)
                      }
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-[hsl(var(--border))] bg-white p-6">
            ...
            <p className="mt-0.5 text-[13px] text-[hsl(var(--muted-foreground))]">
              Rooms, residents, invoices, maintenance, and settings follow the
              selected dorm.
            </p>
            {currentDorm ? (
          ...
          <div className="rounded-xl border border-[hsl(var(--border))] bg-white p-6">
            ...
            <div className="mt-4 space-y-3 text-[13px] text-[hsl(var(--muted-foreground))]">
              <p>Add a dorm to create another workspace.</p>
              <p>
                Make a dorm active before editing its rooms, residents,
                invoices, maintenance, or settings.
              </p>
              <p>Archive removes a dorm from this list but keeps its data.</p>
            </div>
          </div>
        </div>
      </div>
