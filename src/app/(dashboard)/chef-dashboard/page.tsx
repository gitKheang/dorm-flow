...
export default function ChefDashboardPage() {
  ...
  const scheduleWindow = useMemo(() => buildMealScheduleWindow(), []);
  const [selectedDay, setSelectedDay] = useState(
    () => buildMealScheduleWindow()[0]?.dayLabel ?? "Monday",
  );
  const [showAddForm, setShowAddForm] = useState(false);
  const [newMealName, setNewMealName] = useState("");
  const [newMealCategory, setNewMealCategory] =
    useState<(typeof CATEGORIES)[number]>("Breakfast");
  ...
  const selectedScheduleDay =
    scheduleWindow.find((day) => day.dayLabel === selectedDay) ??
    scheduleWindow[0];
  const weekRangeLabel =
    scheduleWindow.length > 0
      ? `${scheduleWindow[0].shortLabel} - ${scheduleWindow[scheduleWindow.length - 1].shortLabel}`
      : "Next 7 days";
  ...
  const subscribedResidents = workspace.tenantMealPreferences
    .filter((preference) => {
      const tenant = currentDormTenants.find(
        (item) => item.id === preference.tenantId,
      );
    ...
    .filter(
      (preference) =>
        countEnabledMealSelections(preference.selections ?? []) > 0,
    );
  const selectedDayDemand = getMealSelectionCountsForDay(
    subscribedResidents,
    selectedDay,
  );
  const breakfastDemand = selectedDayDemand.breakfast;
  const lunchDinnerDemand = selectedDayDemand.lunch + selectedDayDemand.dinner;

  const dietaryBreakdown = meals.reduce(
    (acc, m) => {
      m.dietary.forEach((d) => {
        acc[d] = (acc[d] || 0) + 1;
      });
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to update the meal status.";
      toast.error(message);
    }
  }
  ...
  function handleDeleteMeal(id: string) {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to remove the meal.";
      toast.error(message);
    }
  }
  ...
  function handleAddMeal(e: React.FormEvent) {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Unable to add the meal.";
      toast.error(message);
    }
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-0.5 text-[14px] text-[hsl(var(--muted-foreground))]">
            Kitchen Workspace — {session?.dormName ?? "Sunrise Dormitory"}
          </p>
        </div>
        <FeatureGateNotice
          eyebrow={reason === "plan" ? "Premium feature" : "Meal service off"}
          title={
            reason === "plan"
              ? "Kitchen tools require Premium for this dorm"
              : "Meal service is turned off for this dorm"
          }
          description={
            reason === "plan"
              ? "Upgrade this dorm to manage meals and resident meal notes."
              : "Turn meal service back on in settings to manage meals and resident meal notes."
          }
          ctaLabel={
            reason === "plan" ? "Upgrade to Premium" : "Open dorm settings"
          }
          ctaHref={
            reason === "plan"
              ? buildPremiumCheckoutHref({
                  source: "chefDashboard",
                  returnTo: "/chef-dashboard",
                })
              : "/settings?tab=dorm"
          }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl p-5 border border-[hsl(var(--border))] flex flex-col gap-2">
          ...
          <p className="text-2xl font-700 text-[hsl(var(--foreground))]">
            {subscribedResidents.length}
          </p>
          <p className="text-[12px] text-[hsl(var(--muted-foreground))]">
            Residents with meal selections
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-3 bg-white rounded-xl border border-[hsl(var(--border))] p-6 space-y-5">
          ...
          {showAddForm && (
            <form
              ...
              className="p-4 bg-[hsl(var(--muted)/0.5)] rounded-lg border border-[hsl(var(--border))] space-y-3"
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <AppSelect
                  ...
                  onChange={(value) =>
                    setNewMealCategory(value as (typeof CATEGORIES)[number])
                  }
                  fullWidth
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="flex-1 py-2 text-[13px] font-medium text-white bg-[hsl(var(--primary))] rounded-lg hover:bg-[hsl(var(--primary)/0.9)] transition-colors"
                >
                  Add Meal
                </button>
                <button
          ...
          <div className="space-y-3">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat}>
                  <p
                    className={`inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full border mb-2 ${categoryColors[cat]}`}
                  >
                  ...
                  <div className="space-y-2">
                    {catMeals.map((meal) => (
                      <div
                        key={meal.id}
                        className="flex items-center gap-3 p-3 rounded-lg border border-[hsl(var(--border))] hover:bg-[hsl(var(--muted)/0.3)] transition-colors"
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {meal.dietary.map((d) => (
                              <span
                                key={d}
                                className="text-[11px] bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))] px-1.5 py-0.5 rounded"
                              >
                                {d}
                              </span>
                            ))}
                        ...
                        <AppSelect
                          ...
                          onChange={(value) =>
                            handleStatusChange(
                              meal.id,
                              value as MealItemRecord["status"],
                            )
                          }
            ...
            {todayMeals.length === 0 && (
              <div className="text-center py-8">
                <UtensilsCrossed
                  size={32}
                  className="text-[hsl(var(--muted-foreground))] mx-auto mb-2 opacity-40"
                />
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-[hsl(var(--border))] p-5 space-y-4">
            <h3 className="text-[14px] font-semibold text-[hsl(var(--foreground))]">
              {selectedScheduleDay
                ? `${selectedScheduleDay.dayLabel} Summary`
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-[13px]">
                ...
                <span className="font-medium">
                  {todayMeals.reduce((s, m) => s + m.servings, 0)}
                </span>
              </div>
              <div className="flex justify-between text-[13px]">
                <span className="text-[hsl(var(--muted-foreground))]">
              ...
              <div className="flex justify-between text-[13px]">
                ...
                <span className="font-medium">{selectedDayDemand.total}</span>
              </div>
              <div className="flex justify-between text-[13px]">
                <span className="text-[hsl(var(--muted-foreground))]">
                  Avg calories
          ...
          <div className="bg-white rounded-xl border border-[hsl(var(--border))] p-5 space-y-4">
            ...
            <div className="space-y-3">
              {subscribedResidents.length === 0 && (
                <p className="text-[13px] text-[hsl(var(--muted-foreground))]">
                  No resident meal selections yet.
                </p>
              )}
